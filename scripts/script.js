  ...
  const nameUser = localStorage.getItem("nomeDoMano");
  console.log(nameUser);
if (nameUser) {
    document.querySelector('.img-login').style.display = 'none'
    const li = document.createElement('li')
    const a = document.createElement('a')
    a.innerHTML = nameUser.toUpperCase()
    li.append(a)
    document.querySelector('.ul-header').append(li)
}
...
function modeloEscolhido(idCarro) {
  fetch("data/carros.json")
    ...
    .then((data) => {
      const carroSelecionado = data.carros.find(
        (carro) => carro.id === idCarro
      );
      if (carroSelecionado) {
        localStorage.setItem(
          "carroEscolhido",
          JSON.stringify(carroSelecionado)
        );
